...
{
    [Info("Stack Size Controller", "AnExiledDev/Optimized by Sery", "4.1.3")]
    ...
    class StackSizeController : CovalencePlugin
    {
        ...
        {
            public bool RevertStackSizesToVanillaOnUnload = false;
...
private void OnEntitySpawned(Minicopter heli)
{
    ...
    NextTick(() =>
    {
        ...
        var fuelContainer = heli.GetComponent<StorageContainer>();
        if (fuelContainer == null || fuelContainer.inventory == null || fuelContainer.inventory.itemList == null || fuelContainer.inventory.itemList.Count == 0)
            return;

        var fuelItem = fuelContainer.inventory.itemList.FirstOrDefault();
        if (fuelItem == null || fuelItem.info == null || fuelItem.info.shortname != "lowgradefuel" || fuelItem.amount != fuelItem.info.stackable / 5)
            return;
